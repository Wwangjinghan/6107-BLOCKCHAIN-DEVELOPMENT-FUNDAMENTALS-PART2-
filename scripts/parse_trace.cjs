...
async function parseTrace() {
    ...
    // 5.1 生成 callFrames 与 stepToFrameId（frame-aware mapping）
    const callFrames = [];
    ...
    rows.forEach((row, idx) => {
        // assign current frame id for this step
        ...
        if (row.isCall) {
            ...
            if (frameStack.length > 0) {
                frameStack[frameStack.length - 1].children.push(frame);
            } else {
                callFrames.push(frame);
            }
            frameStack.push(frame);
            // mark this step as belonging to the new frame
            stepToFrameId[idx] = frame.id;
        }

        if (row.op === 'RETURN' || row.op === 'REVERT') {
            while (frameStack.length > 0 && frameStack[frameStack.length - 1].depth >= row.depth) {
                frameStack.pop();
            }
        }
    });

    // 5.2 在 rows 中补充 storageSlotName（若可用）
    const sstoreByStep = {};
    sstores.forEach(s => { sstoreByStep[s.step] = s; });
    rows.forEach(r => {
        if (r.isStorage && sstoreByStep[r.step]) {
            r.storageSlotName = '0x' + sstoreByStep[r.step].slot;
        }
    });
